...
func PrintResult(b *bucket.Bucket, json bool) {
	...
	// 추가
	// mt.Println("DEBUG - Enumerated object count:", len(b.Objects))
	// for _, obj := range b.Objects {
	//	fmt.Printf("DEBUG → %s (%d bytes)\n", obj.Key, obj.Size)
    // 객체 목록도 INFO 레벨로 출력 (DEBUG → INFO 변경)
	if b.ObjectsEnumerated && len(b.Objects) > 0 {
		for _, obj := range b.Objects {
			fmt.Printf(" [object] %s (%s)\n", obj.Key, humanize.Bytes(obj.Size))
		}
	}
}
